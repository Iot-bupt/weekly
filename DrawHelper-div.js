...
var DrawHelper = (function () {
    ...
    _.prototype.startDrawingMarker = function (options) {
        ...
        mouseHandler.setInputAction(function (movement) {
            ...
            if (position != null) {
                ...
                if (cartesian) {
                    tooltip.showAt(position, "<p>请添加标记位置: </p>" + getDisplayLatLngString(ellipsoid.cartesianToCartographic(cartesian)));
                } else {
                    tooltip.showAt(position, "<p>请在正确位置添加.</p>");
                }
            }
        }, Cesium.ScreenSpaceEventType.MOUSE_MOVE);
    ...
    _.prototype.startDrawingPolyshape = function (isPolygon, options) {
		...
		//距离量测  byXin 17-09-12
        var mouseHandler = new Cesium.ScreenSpaceEventHandler(scene.canvas);
        var lastPointPosition = null;
        var lastlabelID = null;
        var distanceSum = 0;//总距离
        var tempdistance = 0;
        var currentDistance = 0;//当前距离
        ...
        mouseHandler.setInputAction(function (movement) {
            if (movement.position != null) {
                ...
                if (cartesian) {
                    // first click
                    var thisPointPosition = lastPointPosition;
                    lastPointPosition = cartesian.clone();
                    ...
                    if (positions.length >= minPoints) {//点击的点的数量大于1，开始画线了
                        poly.positions = positions;
                        poly._createPrimitive = true;

                        var Line = viewer.entities.add({//画完显示的线
                            name: 'disLine',
                            polyline: {
                                positions: [thisPointPosition, lastPointPosition],
                                width: 2,
                                material: Cesium.Color.WHITE
                            }
                        });
                        //sh.measureId.push(Line.id);

                        var Linedistance = getDis(thisPointPosition, lastPointPosition);
                        distanceSum += Linedistance;
                        tempdistance = distanceSum;
                        var disLabel = viewer.entities.add({//距离的label
                            name: "预测点时间",
                            position: lastPointPosition,
                            label: {
                                text: distanceSum.toFixed(3).toString() + "km",
                                font: '16px Helvetica',
                                fillColor: new Cesium.Color(255, 0, 0),
                                outlineColor: new Cesium.Color(255, 0, 0),
                                outlineWidth: 1,
                        ...
                        //sh.measureId.push(disLabel.id);
                        lastlabelID = disLabel.id;
        ...
        function getDis(pos1, pos2) {
            ...
            dis = Math.sqrt(xsq + ysq + zsq);
            return parseFloat((dis / 1000));
        }
        ...
        mouseHandler.setInputAction(function (movement) {
            ...
            if (position != null) {
                if (positions.length == 0) {
                    ...
                    tooltip.showAt(position, "<p>请添加起始测距点</p>");
                } else {
                    var cartesian = scene.camera.pickEllipsoid(position, ellipsoid);
                    if (cartesian) {
                        positions.pop();
                        ...
                        var currentP = cartesian.clone();
                        var dist = getDis(lastPointPosition, currentP);

                        currentDistance = tempdistance + dist;                    
                        tooltip.showAt(position, "<p>当前总距离为：" + currentDistance.toFixed(3) + "km</p>");
                ...
                if (positions.length < minPoints + 2) {
                    ...
                    if (cartesian) {
                        ...
                        if (typeof options.callback == 'function') {
                            ...
                            viewer.entities.removeById(lastlabelID);
    ...
    // _.prototype.startDrawingPolygon = function(options) {
    //     var options = copyOptions(options, defaultSurfaceOptions);
    //     this.startDrawing123(true, options);
    // }

    _.prototype.startDrawing123 = function( options) {
        ...
        var primitives = scene.primitives;
        var tooltip = this._tooltip;
        var lastPointPosition = null;//zuihou
        var lastlabelID = null;
        var totalArea = 0;//总面积
        var tempArea = 0;
            ...
            if(movement.position != null) {
                ...
                if (cartesian) {
                    ...
                    if(positions.length >= minPoints) {
                        ...
                        var area = computeAreaOfTriangle(startPosition,thisPointPosition,lastPointPosition);
                        totalArea += area;
                        //tempArea = totalArea;
                    }
                    // add new point to polygon
    ...
    {
        var a = Cesium.Cartesian3.distance(new Cesium.Cartesian3(pos1.x, pos1.y, pos1.z), new Cesium.Cartesian3(pos2.x, pos2.y, pos2.z));
        var b = Cesium.Cartesian3.distance(new Cesium.Cartesian3(pos2.x, pos2.y, pos2.z), new Cesium.Cartesian3(pos3.x, pos3.y, pos3.z));
        var c = Cesium.Cartesian3.distance(new Cesium.Cartesian3(pos3.x, pos3.y, pos3.z), new Cesium.Cartesian3(pos1.x, pos1.y, pos1.z));

        var S = (a + b + c) / 2;

        return Math.sqrt(S * (S - a) * (S - b) * (S - c))/1000000;
     }
            ...
            if(position != null) {
                if(positions.length == 0) {
                    ...
                    if (cartesian) {
                        ...
                        markers.getBillboard(positions.length - 1).position = cartesian;
                        // show tooltip
                        tooltip.showAt(position, "<p>添加新的点 (" + positions.length + ")</p>" + (positions.length > minPoints ? "<p>双击结束</p>" : ""));
                    }
                }
            }
        }, Cesium.ScreenSpaceEventType.MOUSE_MOVE);
                ...
                if(positions.length < minPoints + 2) {
                    ...
                    if (cartesian) {
                        ...
                        if(typeof options.callback == 'function') {
                            ...
                            var index = positions.length - 1;
                            options.callback(positions);
            tooltip.showAt(position, "<p>总面积为：" + (totalArea).toFixed(3) + "平方公里</p>");
                        }
                    }
                }
            }
            ...
            if (movement.position != null) {
                ...
                if (cartesian) {
                    if (circle == null) {
                        ...
                        _self.stopDrawing();
                        //tooltip.showAt(position, "<p>总面积为：" + 'radius'.toFixed(3) + "平方公里</p>");
                    }
                }
            }
        }, Cesium.ScreenSpaceEventType.LEFT_DOWN);
    ...
    _.DrawHelperWidget = (function () {
        ...
        function _(drawHelper, options) {
            ...
            addIcon('polyline', options.polylineIcon, '距离测量', function () {
            ...
            addIcon('polygon', options.polygonIcon, '面积测量', function() {
               drawHelper.startDrawing123({
                   callback: function(positions) {
            ...
            addIcon('circle', options.circleIcon, '区域测量', function() {
            ...
            toolbar.appendChild(div);
            addIcon('clear', options.clearIcon, '移除所有', function () {
